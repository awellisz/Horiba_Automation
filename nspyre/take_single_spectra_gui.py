"""
GUI to take a single spectrum at a time with the Horiba
...
import experiments.Spectra.take_single_spectra

_logger = logging.getLogger(__name__)


class SpectrometerWidget(ExperimentWidget):
	def __init__(self):
		...
		params_config = {
			"dataset": {
				...
				"widget": QtWidgets.QLineEdit("spec0"),
			},
			# "autosave": {
			...
			"filename": {
				...
				"widget": QtWidgets.QLineEdit("%gg_%ts_%wnm_spectrum_%n")
			},
			# capture settings
			"exposure_s": {
		...
		super().__init__(
			...
			module = experiments.Spectra.take_single_spectra,
			cls = 'SingleSpectraMeasurement',
			fun_name = 'take_one_spectrum',
			title="Spectra",
		...
		self._timer.start(50) # check every 100 ms
...
# TODO: FIND A WAY TO SHOW MULTIPLE SPECTRA AT ONCE?
class FlexLinePlotWidgetWithSpectraDefaults(FlexLinePlotWidget):
	"""FlexLine plot w/ defaults for looking at spectra"""
	def __init__(self):
		super().__init__()

		self.add_plot('spectrum', series='spectra', scan_i='', scan_j='', processing='Average')

		legend = self.line_plot.plot_widget.addLegend()
		legend.setOffset((-10, -50))

		self.datasource_lineedit.setText('spec0')
